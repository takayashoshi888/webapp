...
function generateCalendar(year, month, records) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const dayElement = document.createElement('div');
        dayElement.className = `h-12 border border-gray-200 flex items-center justify-center relative ${isAttended ? 'bg-green-100' : 'bg-gray-50'}`;
        
        dayElement.innerHTML = `
            <span class="text-sm font-medium">${day}</span>
            ${isAttended ? '<div class="absolute bottom-1 right-1 w-2 h-2 bg-green-500 rounded-full"></div>' : ''}
        `;
